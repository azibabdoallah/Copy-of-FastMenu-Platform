...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ config: initialConfig, onUpdate, onLogout }) => {
  ...
  const refreshData = async () => {
      // استخدام المعرف الحقيقي لتحديث البيانات
      if (!config.id) return;
      const newData = await getRestaurantConfig(config.id);
      setConfig(newData);
      setSettingsForm(newData);
  };

  const fetchOrders = async (isBackground = false) => {
    // التأكد من وجود المعرف قبل محاولة الجلب
    if (!config.id) return;
    
    if (!isBackground) setLoadingOrders(true);
    try {
        const data = await getOrders(config.id);
