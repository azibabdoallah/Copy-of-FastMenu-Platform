...
const CustomerMenu: React.FC<CustomerMenuProps> = ({ config: initialConfig }) => {
  ...
  // Default to 'ar', no setter exposed to UI
  ...
  // Handle Offer Carousel Auto-scroll
  useEffect(() => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Logic includes 'offers' as a section
      const sections = [];
  ...
  const scrollToCategory = (id: string) => {
    ...
    if (element) {
      const y = element.getBoundingClientRect().top + window.pageYOffset - 120; // Reduced offset for compact header
  ...
  const addToCart = (dish: Dish, quantity: number = 1) => {
   ...
   const handleOfferClick = (offer: Offer) => {
      // Convert Offer to Dish format for display in modal
      ...
      // Open the modal instead of adding directly
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    try {
      await submitOrder({
        ...
        table_number: orderType === 'delivery' ? 'توصيل' : tableNumber, // Use 'Delivery' as placeholder for table number in list
        items: cart,
        total: cartTotal,
        type: orderType,
        phone: customerPhone,
  ...
  // Logic to check if restaurant is open based on WorkingHours
  const checkIfOpen = (): boolean => {
      if (!currentConfig.workingHours) return true; // Default to open if not configured
      ...
      // Handle cases where closing time is next day (e.g. 02:00 AM)
  ...
  return (
    <div className="min-h-screen bg-white pb-20 font-sans text-slate-900">
      ...
      {/* 1. Header & Navigation Control (Compact) */}
      <div className="px-3 pt-3 flex justify-between items-center bg-white mb-2 z-30 relative">
         ...
         <button
            onClick={handleExit}
            className="bg-gray-50 text-gray-700 p-2 rounded-full hover:bg-gray-100 transition-colors shadow-sm border border-gray-100"
        >
             {isOwner ? <LogOut size={16} /> : <ArrowRight size={16} className={language === 'ar' ? 'rotate-0' : 'rotate-180'} />}
        </button>
      </div>

      {/* 2. Offers Banner (Carousel) */}
                ...
                <>
                    ...
                    <div className="absolute bottom-2 left-0 right-0 flex justify-center gap-1.5 z-10">
                        {activeOffers.map((_, idx) => (
                            <div 
                                key={idx} 
                                className={`w-1.5 h-1.5 rounded-full transition-all ${currentOfferIndex === idx ? 'bg-primary w-3' : 'bg-white/70'}`} 
                            />
                        ))}
                    </div>
                </>
      ...
      {/* 3. Restaurant Profile Header (Compact + Overlap) */}
      <div className="px-4 mb-2 flex items-end justify-between relative z-10">
          <div className="flex items-end gap-3">
              {/* Logo: Bigger, Negative Margin, White Border */}
              ...
              <div className="flex flex-col items-start pb-1">
                  ...
                  {/* Delivery Toggle - ADDED HERE */}
                  {isOrderingEnabled && (
                      <div className="flex bg-gray-100 p-0.5 rounded-lg border border-gray-200">
                          <button 
                             onClick={() => setOrderType('dine_in')}
                             className={`px-3 py-1 rounded-md text-[10px] font-bold flex items-center gap-1 transition-all ${orderType === 'dine_in' ? 'bg-white text-slate-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                          >
                              <Utensils size={10} /> {t.dineIn}
                          </button>
                          <button 
                             onClick={() => setOrderType('delivery')}
                             className={`px-3 py-1 rounded-md text-[10px] font-bold flex items-center gap-1 transition-all ${orderType === 'delivery' ? 'bg-white text-black shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                          >
          ...
          <div className="flex flex-col items-end gap-2 pb-1">
             ...
             <div className="flex gap-2">
                 ...
                 {currentConfig.socials.tiktok && (
                     <a href={getSocialUrl('tiktok', currentConfig.socials.tiktok)} target="_blank" rel="noreferrer" className="text-gray-400 hover:text-black bg-gray-50 p-1.5 rounded-full transition-colors">
                        <TikTokIcon />
                     </a>
                 )}
             </div>

             {/* Google Maps Rate Button */}
             {currentConfig.socials.googleMaps && (
                 <a 
                    href={currentConfig.socials.googleMaps}
                    target="_blank" 
                    rel="noreferrer"
                    className="flex items-center gap-1.5 bg-yellow-50 text-yellow-600 border border-yellow-100 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-yellow-100 transition-colors"
                 >
      ...
      {/* 4. Category Pills (Sticky & Compact) */}
      <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm py-2 border-b border-gray-50 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.02)]">
        <div className="flex overflow-x-auto no-scrollbar px-3 gap-2">
          {/* Special Chip for Offers */}
          {activeOffers.length > 0 && (
             <button
              onClick={() => scrollToCategory('offers')}
              className={`whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold transition-all border flex items-center gap-1 ${
                activeCategory === 'offers'
                  ? 'bg-black border-black text-primary shadow-md' 
                  : 'bg-yellow-50 border-yellow-100 text-black hover:bg-yellow-100'
              }`}
            >
            ...
            <button
              ...
              className={`whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold transition-all border ${
                activeCategory === cat.id 
                  ...
                  : 'bg-[#f8f8f8] border-transparent text-gray-500 hover:bg-gray-100'
              }`}
            >
              {cat.name}
            </button>
      ...
      <div className="px-3 py-2 min-h-[500px]">
        ...
        {/* Offers Section (Rendered as Dishes) */}
        {activeOffers.length > 0 && (
      ...
      {currentConfig.socials.whatsapp && (
        <a 
            href={`https://wa.me/${currentConfig.socials.whatsapp}`} 
            target="_blank" 
            rel="noreferrer"
            className="fixed bottom-6 left-5 z-30 bg-[#25D366] rounded-full shadow-lg hover:scale-110 transition-transform flex items-center justify-center w-12 h-12"
        >
            <WhatsAppIcon />
        </a>
      )}
      ...
      {isOrderingEnabled && cart.length > 0 && (
        <button 
          ...
          className="fixed bottom-6 right-5 z-30 bg-black text-white px-5 py-3 rounded-full shadow-2xl flex items-center gap-3 animate-in slide-in-from-bottom border border-gray-800"
        >
          <div className="bg-primary text-black w-5 h-5 flex items-center justify-center rounded-full text-[10px] font-bold">
                     ...
                     {daysOrder.map(day => {
                         ...
                         return (
                             <div key={day} className="flex justify-between items-center p-3 border-b border-gray-50 last:border-0 hover:bg-gray-50 rounded-lg">
                                 ...
                                 <span className={`text-sm font-bold ${schedule.isOpen ? 'text-gray-900' : 'text-red-500'}`}>
                                     {schedule.isOpen 
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-end md:justify-center bg-black/50 backdrop-blur-sm animate-in fade-in" onClick={() => setIsCartOpen(false)}>
          <div 
            className="bg-white w-full md:w-[400px] md:h-auto h-[80vh] md:rounded-2xl rounded-t-2xl flex flex-col shadow-2xl animate-in slide-in-from-bottom"
              ...
              {isCheckingOut ? (
                <form onSubmit={handleCheckout} className="space-y-3 animate-in fade-in">
                  ...
                  <input 
                    ...
                    className="w-full border rounded-lg px-3 py-2 text-sm focus:border-black outline-none"
                    value={customerName}
                  ...
                  {orderType === 'dine_in' ? (
                      ...
                      <>
                        ...
                        <textarea 
                            ...
                            className="w-full border rounded-lg px-3 py-2 text-sm focus:border-black outline-none h-20 resize-none"
                  ...
                  <div className="flex gap-2 pt-2">
                    <button 
                      ...
                      className="flex-1 py-2 text-gray-600 text-sm font-medium hover:bg-gray-200 rounded-lg"
                    >
                      {t.back}
                    </button>
                    <button 
                      ...
                      className="flex-[2] py-2 bg-black text-white text-sm font-bold rounded-lg shadow hover:bg-gray-800 disabled:opacity-50"
                    >
                ...
                <button 
                  ...
                  className="w-full py-3 bg-black text-primary font-bold rounded-xl shadow-lg hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  {t.completeOrder}
                </button>
              )}
      ...
      {selectedDish && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/50 p-0 md:p-4 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setSelectedDish(null)}>
          <div 
            className="bg-white w-full md:max-w-lg md:rounded-2xl rounded-t-2xl overflow-hidden max-h-[90vh] overflow-y-auto flex flex-col shadow-2xl animate-in slide-in-from-bottom duration-300"
            onClick={e => e.stopPropagation()}
          >
            ...
            <div className="p-6">
              ...
              <button 
                ...
                className={`w-full font-bold py-3.5 rounded-xl shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2 ${
                  isOrderingEnabled 
                  ? 'bg-black hover:bg-gray-800 text-primary' 
